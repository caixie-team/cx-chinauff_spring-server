...
module.exports = class extends Base {
  ...
  async takeAction () {
    if (this.isPost) {
      ...
      const accountModel = this.model('account')
      const account = await accountModel
      // 账户存在，拉取账户的信息，并验证账户是否已登录了 crm
      // const account = await this.model('account')
      // 账户不存在，创建账户，并验证账户是否已登录了 crm
  ...
  async getCrmUserInfo () {
    ...
    // "msg": "用户已登录",
    // "userId": 4347415014,
    // "cardNo": "210900000004026919"
    const memberInfo = (await this.got(
