/* eslint-disable no-undef,guard-for-in,max-depth */
...
module.exports = class extends Base {
   ...
   */
  async exists (name) {
    const isExists = await this.model('category').where({name: name}).find()
    return think.isEmpty(isExists)
  }
   ...
   */
  async defaultAction () {
    this.meta_title = '实验类别管理'
    this.active = `${this.ctx.module}/category/default`

    let defaultCategory = this.config('setup.default_category')
    // let defaultMold = this.config('setup.default_mold')
    // return this.success(defaultCategory)
    if (think.isEmpty(defaultCategory)) {
      defaultCategory = 1
    }
    const where = {}
    // 默认为系统模型
  ...
  async newAction () {
    ...
    if (this.isPost) {
      ...
      if (think.isEmpty(data.pid)) {
        data.pid = defaultCategory
      }
      // if 内容模型为空则采用默认内容模型
      if (think.isEmpty(data.model)) {
        data.model = this.config('setup.default_model')
      }
      if (think.isEmpty(data.mold)) {
        // 系统模型
        data.mold = 0
      }

      // 验证同级别下分类名称
      data.name = think.isEmpty(data.name) ? slugify(data.title, '_') : data.name
      const isExists = await this.db.exists(data.name)
      if (isExists === false) {
        return this.fail('同级别下,分类名称不能重复!')
      }
      // 添加分类内容
      let res = await this.db.addCategory(data)
      if (res) {
        // return this.display('admin/category/default')
        return this.success({res, name: '分类添加成功', url: '/admin/category/default'})
      } else {
      ...
      this.action = '/admin/category/new'
      // 获取模版列表（pc）
  ...
  async addAction () {
    ...
    if (this.isPost) {
      ...
      // 验证同级别下分类名称
      data.name = think.isEmpty(data.name) ? slugify(data.title) : data.name
      const isExists = await this.db.exists(data.name)
      if (isExists === false) {
        return this.fail('同级别下,分类名称不能重复!')
      }
      // 添加分类内容
      const res = await this.db.addCategory(data)
      if (res) {
  ...
  async oldAddAction () {
    if (this.isPost) {
      const data = this.post()
      if (parseInt(data.mold, 10) !== 2) {
        // 表单验证
        if (think.isEmpty(data.model)) {
          return this.fail('至少要绑定一个模型！')
        }
        if (parseInt(data.mold, 10) === 0) {
  ...
  async editAction () {
    if (this.isPost) {
      ...
      // 验证同级别下分类名称
      data.name = think.isEmpty(data.name) ? slugify(data.title, '_') : data.name
      const isExists = await this.db.exists(data.name, data.id)
      if (isExists === false) {
        return this.fail('同级别下,分类名称不能重复!')
      }

      const res = await this.db.save(data)
      if (res) {
        return this.success({name: '更新成功！', url: '/admin/category/default'});
      } else {
        return this.fail('更新失败！')
      }
    } else {
      const id = this.get('cid')
      const info = await this.db.find(id)
      ...
      this.active = 'admin/category/default'
