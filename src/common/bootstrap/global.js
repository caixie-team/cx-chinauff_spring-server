/**
 * 全局函数文件，将会在服务启动时加载
 * 可以在 controllers, models, templates 中直接使用
 */
...
global.get_children = function (nodes, parent, sn = 0) {
  ...
  /* just in case */ jumper++ < 1000) {
    const shift_node = stack.shift();
    const list = [];
    /* 当前子节点列表 */
    const last_static = last.slice(0);
    last = [];
...
global.get_attribute_type = function (type) {
  ...
  const _type = {
    ...
    'htmlfile': ['上传附件(html压缩包)', 'int(10) unsigned NOT NULL'],
  ...
  if (!think.isEmpty(grid.href)) {
    ...
    for (const link of links) {
      ...
      if (matches) {
        ...
        const href1 = `/${module}/${controller}/${query}`;
        if (href == '[DELETE]') {
          val.push('<a href="' + href1 + '" class="ajax-get confirm">' + show + '</a> ');
        } else {
          val.push('<a href="' + href1 + '" class="">' + show + '</a> ');
        }
      }
    }
    value = val.join(' ');
