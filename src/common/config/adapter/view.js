/* eslint-disable no-undef,no-tabs,new-cap */
const nunjucks = require('think-view-nunjucks');
// const nunjucks = require('../../adapter/nunjucks')
const path = require('path');
module.exports = {
  type: 'nunjucks',
  common: {
    viewPath: path.join(think.ROOT_PATH, 'view'),
    // viewPath: path.join(think.ROOT_PATH, 'pages'),
    sep: '/',
    extname: '.html'
  },
  nunjucks: {
    ...
    beforeRender: (env, nunjucks, config) => {
      ...
      env.addFilter('age', (birthday) => {
        const moment = require('moment');
        const text = moment(birthday).fromNow();
      ...
      env.addFilter('evaluation_format', (status) => { // 获取数据的状态操作
        switch (status) {
          case 0:
            return `<span class="label label-default">待评</span>`
          case 1:
            return `<span class="label label-success">已评</span>`
          default:
            return false;
        }
      });
      ...
      env.addFilter('get_realname', async (uid, callback) => {
        const data = await get_realname(uid);
        callback(null, data);
      }, true);

      env.addFilter('get_vote_text', (vote) => {
        // const data = await get_realname(uid);
        switch (vote) {
          case 1: {
            return '一般'
      ...
      env.addFilter('show_ad', async (spaceid, type, callback) => {
        let res = '';
        try {
          const data = await think.addonModel('ad_space', 'ad').showad(spaceid);
